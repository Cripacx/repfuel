...
4. **Projektregel repfuel:** zusammen mit dem craft-floor immer auch
   `.claude/skills/apple-design/SKILL.md` laden. Sie ist in diesem Repo die
   verbindliche Grundlage für Interaktion, Motion und Materialität und geht bei
   Konflikten über allgemeine Motion-Ratschläge — konkret: Feedback auf
   pointer-down statt erst auf click, 1:1-Tracking bei Gesten, unterbrechbare
   Animationen aus dem Präsentationswert, Springs statt fester Durations für
   alles Anfassbare (kritisch gedämpft als Default, Bounce nur nach echtem
   Momentum), symmetrische Ein-/Ausgangspfade, größenabhängiges
   Tracking/Leading, und `prefers-reduced-motion` / `-transparency` /
   `prefers-contrast` als echte Varianten statt Totalabschaltung. Die
   Motion-Tokens dafür stehen in `apps/web/src/app.css` (`--dur-press`,
   `--ease-out`, `--ease-bounce`, `--ease-in`, `--tracking-*`, `--leading-*`).
